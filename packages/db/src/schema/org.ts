...
import { check, jsonb, pgPolicy, pgTable, text, unique, uuid } from "drizzle-orm/pg-core";
...
export const externalOrg = pgTable(
  ...
  (t) => [unique("external_org_name_per_tenant").on(t.orgId, t.name), tenantPolicy("external_org")],
).enableRLS();
