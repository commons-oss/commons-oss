...
- `@commons-oss/attendance-tracker` — first installable module. Drizzle schema for `event` (training/match/other, optional partner team, cancellation), `attendance_entry` (3-state: present/excused/absent, with marker user + timestamp), both RLS-scoped via the closed-default contract. `mark` server action upserts entries, autosave only (no Speichern button per locked plan §5.4). i18n catalogs (`messages/{de,en}.json`) for nav labels, statuses, summaries, filter chips. Exports `attendanceTracker` module via `defineModule` plus `schema` for migrations.
- `apps/shell/[org]/today` — Trainer landing (M1 Heute). Picks the next event for any team where the user holds a `coach` role, renders 3-state pill marker per roster row with autosave, optimistic UI, and progress summary. Desktop (≥980px) gets a 2-column grid with a sticky right rail showing current-session metadata and the next 5 upcoming events; mobile collapses to a single column.
- `apps/shell/[org]/sessions` — Trainer history (M2 Verlauf). Past sessions list with date/day, kind label, segmented attendance bar (present/excused/absent), counts, present-rate percentage. Filter chips (Alle / Training / Spiele) keep state in URL search params (`?filter=training|match`) — server-filtered, shareable, bookmarkable, survives reload.
- `apps/shell/[org]/profile` — Minimal Phase 1 profile. Identity card (initials + email + active org), read-only locale display, sign-out form. Mannschaft-Wechsel and Sprachumschalter ship in Phase 2.
- `apps/shell/[org]` — Role-defaulted landing redirect: coaches → `/today`, everyone else → `/profile` (officer surfaces ship in Phase 2).
- `apps/shell/src/personas.ts` — `getUserPersonas(userId, orgId)` resolves `{ isCoach, isOfficer }` from `member_role` joins. Used by the layout to filter module nav by persona.
- `@commons-oss/module` — `NavItem.persona?: 'coach' | 'officer' | 'both'` field. Shell filters nav items per signed-in user's roles before rendering.
- Responsive shell chrome (Option C from locked plan §3): single URL space, role+viewport-aware navigation. Sidebar at md+ (≥768px), bottom-tab below. CSS media queries on a single client component (`_components/chrome-nav.tsx`).
- Dogfood seed: 18 events for FC Musterstadt across U15 + Reserve teams (training + matches, past + future) so /today and /sessions render real data on first boot.
